...
匯入工具只讀取 Excel 欄位與 PDF 的一般文字／條文；**不會匯入圖片，也不會建立網頁表格**。它會忽略 PDF 最上方的封面／版次表格，從第一個「第一章」或「第一條」（日文 `第1章`／`第1条` 亦可）開始讀取。PDF 中可辨識的表格列（Tab、`|` 或連續欄位空白）也會略過。PDF 找不到章節或條文起點時會停止匯入，避免誤把封面資料寫入規程。
...
Admin 在「新增規程」或「編輯草稿」畫面可按「＋ 上傳 PDF」。系統會讀取可搜尋文字的 PDF，優先從第一個「第一章」或「第一條」開始帶入目前選擇的中文或日文草稿；檔名若是 `DHT2-0001＿就業規程＿中文.pdf` 形式，也會自動帶入規程名稱。此動作只填入草稿，不會自動儲存、送審或發布。

PDF 文字載入後，請自行確認規程編號、分類、章節與條文，再由使用者手動輸入「生效日期」並按「儲存草稿」。單檔上限為 10 MB；掃描型 PDF 因沒有文字層，需先 OCR 後才能使用。

### 圖片與條文參照

編輯畫面的「規程圖片」上傳後，系統會依順序標記為「圖一、圖二……」。每一條條文下方的「關聯資料」可選擇 `表格 1`、`圖一` 等項目，系統會在該條文下自動顯示相應的參照註解；公開規程也會顯示相同圖號與圖片說明。
